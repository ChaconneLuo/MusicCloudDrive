...
public class TokenServiceImpl implements TokenService {
    ...
    public void deleteAllToken(String token, String secretKey) {
        var email = JWT.require(Algorithm.HMAC256(secretKey)).build().verify(token).getClaim("email").asString();
    ...
    public void deleteSingleToken(String key) {
        ...
        redisTemplate.opsForValue().set(Const.REDIS_FORBIDDEN_PREFIX + key.substring(Const.REDIS_TOKEN_PREFIX.length() + email.length()), "", Const.TOKEN_OVERTIME, TimeUnit.SECONDS);
    }
